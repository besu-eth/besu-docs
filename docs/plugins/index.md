...
Use plugins to build app-specific chains, integrate Besu with enterprise systems, observe
blockchain activity, analyze transactions, support Layer 2 networks, or add debugging and
operational tooling.

Plugins can use Besu services to query, configure, extend, or replace parts of Besu. They can also
register event callbacks to observe Besu activity such as block imports, transaction pool changes,
logs, and sync status changes.
